...
//
struct VendorOrderStruct: Codable {
    let result: Bool?
    let message, servertime: String?
    let data: [VendorOrderStructObj]
}

// MARK: - Datum
struct VendorOrderStructObj: Codable {
    let purchaseAndLedgerBalanceOrderNoVendordata: [VendorOrderData]
    let ismore: Bool?
}

// MARK: - PurchaseAndLedgerBalanceOrderNoVendordatum
...
import Foundation
import UIKit
@IBDesignable class VendorOrder: BaseCustomView {
    ...
    @IBOutlet weak var collectionView: VendorOrderCollectionView!
        ...
        DataManager.shared.makeAPICall(url: vendorListApi, params: json, method: .POST, success: { (response) in
            ...
            DispatchQueue.main.async {
                do {
                    ...
                    print("Vendor Invoice Data \(self.vendorInvoiceStructObj[0].purchaseAndLedgerBalanceOrderNoVendordata)")
...
extension VendorOrder: UICollectionViewDataSource {
...
extension VendorOrder: UICollectionViewDelegate {
    
}



class VendorOrderCollectionView: UICollectionView {
