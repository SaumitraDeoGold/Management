...
open class SideMenuManager : NSObject {
    ...
    /// Duration of the animation when the men@objc u is dismissed without gestures. Default is 0.35 seconds.
    open static var menuAnimationDismissDuration: Double = 0.35
    
    /// Duration of the remaining animation when the menu is part@objc ially @objc dismissed with gestures. Default is 0.2 seconds.
    ...
    @discardableResult open class func menuAddPanGestureToPresent(toView: UIView) -> UIPanGestureRecognizer {
        ...
        panGestureRecognizer.addTarget(SideMenuTransition.self, action: #selector(SideMenuTransition.handleHideMenuPan(_:)))
