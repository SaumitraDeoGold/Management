...
import Foundation
import UIKit
@IBDesignable class VendorInvoice: BaseCustomView {
    ...
    var vendorInvoiceStruct = [VendorInvoiceStruct]()
    var vendorInvoiceStructObj = [VendorInvoiceStructObj]()
    var vendorListData = [VendorListData]()
    
    @IBOutlet weak var noDataView: NoDataView!
    
    var vendorListApi = ""
    
    @IBOutlet weak var collectionView: VendorInvCollectionView!
    ...
    override func xibSetup() {
        ...
        vendorListApi = "https://test2.goldmedalindia.in/api/getVendorPurchaseAndLedgerBalanceInvoiceNo"
    ...
    @IBAction func clicked_all_invoice(_ sender: UIButton) {
        ...
        allInvoices.from = "vendorInvoice"
    ...
    func apiLastDispatchedMaterial(){
        ...
        DataManager.shared.makeAPICall(url: vendorListApi, params: json, method: .POST, success: { (response) in
            ...
            DispatchQueue.main.async {
                do {
                    self.vendorInvoiceStruct = try JSONDecoder().decode([VendorInvoiceStruct].self, from: data!)
                    self.vendorInvoiceStructObj = self.vendorInvoiceStruct[0].data
                    print("Vendor Invoice Data \(self.vendorInvoiceStructObj[0].purchaseAndLedgerBalanceInvoiceNoVendordata)")
                    self.vendorListData = self.vendorInvoiceStructObj[0].purchaseAndLedgerBalanceInvoiceNoVendordata
...
extension VendorInvoice: UICollectionViewDataSource {
...
extension VendorInvoice: UICollectionViewDelegate {
    
}



class VendorInvCollectionView: UICollectionView {
