...
import Foundation
struct VendorPayStuct: Codable {
    let result: Bool?
    let message, servertime: String?
    let data: [VendorPayStuctObj]
}

// MARK: - Datum
struct VendorPayStuctObj: Codable {
    let purchaseAndLedgerBalancePaymentVendordata: [VendorPayData]
    let ismore: Bool?
}

// MARK: - PurchaseAndLedgerBalancePaymentVendordatum
struct VendorPayData: Codable {
    let slno: Int?
    let voucherNo, date: String?
    let amount: Int?
}
import UIKit
@IBDesignable class VendorPayment: BaseCustomView {
    ...
    @IBOutlet weak var collectionView: VendorPayCollectionView!
    ...
    override func xibSetup() {
        ...
        vendorListApi = "https://test2.goldmedalindia.in/api/getVendorPurchaseAndLedgerBalancePayment"
    ...
    func apiLastDispatchedMaterial(){
        ...
        DataManager.shared.makeAPICall(url: vendorListApi, params: json, method: .POST, success: { (response) in
            ...
            DispatchQueue.main.async {
                do {
                    self.vendorInvoiceStruct = try JSONDecoder().decode([VendorPayStuct].self, from: data!)
                    self.vendorInvoiceStructObj = self.vendorInvoiceStruct[0].data
                    print("Vendor Invoice Data \(self.vendorInvoiceStructObj[0].purchaseAndLedgerBalancePaymentVendordata)")
                    self.vendorListData = self.vendorInvoiceStructObj[0].purchaseAndLedgerBalancePaymentVendordata
...
extension VendorPayment: UICollectionViewDataSource {
...
extension VendorPayment: UICollectionViewDelegate {
    
}



class VendorPayCollectionView: UICollectionView {
