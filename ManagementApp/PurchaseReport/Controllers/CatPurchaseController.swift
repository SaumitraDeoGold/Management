...
//Model Struct...
struct CatPurchaseData: Codable {
    let result: Bool?
    let message, servertime: String?
    let data: [CatPurchaseObj]
}

// MARK: - Datum
struct CatPurchaseObj: Codable {
    let catId, catName, amount: String?
}

import UIKit
class CatPurchaseController: BaseViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    var catPurchaseData = [CatPurchaseData]()
    var catPurchaseObj = [CatPurchaseObj]()
    var filteredItems = [CatPurchaseObj]()
    ...
    override func viewDidLoad() {
        ...
        self.title = "Categorywise Purchase"
        addSlideMenuButton()
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.section == 0 {
            if indexPath.row == 1 {
                ...
                cell.contentLabel.text = "Category Name"
            }
        }
        ...
        if indexPath.section == self.filteredItems.count + 1 {
            ...
            {
                if indexPath.row == 0 {
                    cell.contentLabel.text = self.filteredItems[indexPath.section-1].catName ?? "-"
                } else if indexPath.row == 1 {
...
//                    }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if indexPath.row == 0 && indexPath.section == 0{
            ...
            popup.modalPresentationStyle = .overFullScreen
            popup.delegate = self
            popup.fromPage = "CatPurchase"
            popup.catPurchase = catPurchaseObj
            popup.tempcatPurchaseObj = catPurchaseObj
            self.present(popup, animated: true)
        }else if indexPath.section != 0{
            let storyboard = UIStoryboard(name: "VendorPurchase", bundle: nil)
            let destViewController = storyboard.instantiateViewController(withIdentifier: "PartyPurchaseController") as! PartyPurchaseController
            destViewController.catId = filteredItems[indexPath.section-1].catId!
            destViewController.finYear = finYear
    ...
    func showParty(value: String,cin: String) {
        ...
        filteredItems = self.catPurchaseObj.filter { $0.catName == value }
        self.collectionView.reloadData()
        self.collectionView.collectionViewLayout.invalidateLayout()
    }
    ...
    func apiLastDispatchedMaterial(){
        ...
        let json: [String: Any] =  ["CIN":UserDefaults.standard.value(forKey: "userCIN") as! String, "Category":UserDefaults.standard.value(forKey: "userCategory") as! String,"ClientSecret":"ohdashfl","Finyear":finYear]
            print("Vendor Invoice Params powise \(json)")
            DataManager.shared.makeAPICall(url: "https://test2.goldmedalindia.in/api/getcatwisepurchase", params: json, method: .POST, success: { (response) in
                let data = response as? Data
    
                DispatchQueue.main.async {
                    do {
                        self.catPurchaseData = try JSONDecoder().decode([CatPurchaseData].self, from: data!)
